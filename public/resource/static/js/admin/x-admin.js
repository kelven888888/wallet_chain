...
*/
	...
	element.on('nav(side)', function(elem) {
		var url   = elem.parent().find('a').attr('lay-href');
      //  alert(url)
		if (url) {
			console.log("onclick nav")
			load_pagenav(url,"top");
		}
	});
...
function reload_page() {
	if ($.cookie('x_iframe_url')) {
		...
		console.log($.cookie('x_iframe_url'))
		load_page($.cookie('x_iframe_url'));
	}
}
function load_pages(url) {
    ...
    */
	...
	var count =$.cookie('tabpage')
	var url =$.cookie('x_iframe_url')
	console.log(url)
	//$('.layadmin-iframe').eq(count).attr("src",url);
	self.window.location.href=url
	//$('.layadmin-iframe ')[count].src=url
...
*/
	...
	// console.log(count)
	// console.log(count)
	// top.$('.layadmin-iframe').eq(count).attr("src",url);
	// top.$('.x-iframe').eq(count).attr("src", url);
	if (url==""){
