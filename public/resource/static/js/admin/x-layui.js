...
// 发异步，把数据提交给服务端
var is_submit = true;
function ajax_post_by_fn(url, data, tourl, is_alert, is_parent, is_close, fn) {
	if (!is_submit) {
		return false;
	}
...
var is_submit = true;
function ajax_post_by_json(url, data, tourl, is_alert, is_parent, is_close) {
	console.log(data)
	if (!is_submit) {
		return false;
	}
