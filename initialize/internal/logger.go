...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if global.SHOP_CONFIG.Mysql.LogZap != "" {
		switch len(data) {
		case 0:
			global.SHOP_LOG.Info(message)
		case 1:
			global.SHOP_LOG.Info("gorm", zap.Any("src", data[0]))
		case 2:
			global.SHOP_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			global.SHOP_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
		}
		return
	}
