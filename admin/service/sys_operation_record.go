...
import (
	...
	systemReq "wallet_chain.com/admin/model/common/request"
)
...
func (operationRecordService *OperationRecordService) CreateSysOperationRecord(sysOperationRecord model.SysOperationRecord) (err error) {
	err = global.SHOP_DB.Create(&sysOperationRecord).Error
	return err
}
...
func (operationRecordService *OperationRecordService) DeleteSysOperationRecordByIds(ids request.IdsReq) (err error) {
	err = global.SHOP_DB.Delete(&[]model.SysOperationRecord{}, "id in (?)", ids.Ids).Error
	return err
}
...
func (operationRecordService *OperationRecordService) DeleteSysOperationRecord(sysOperationRecord model.SysOperationRecord) (err error) {
	err = global.SHOP_DB.Delete(&sysOperationRecord).Error
	return err
}
...
func (operationRecordService *OperationRecordService) GetSysOperationRecord(id uint) (sysOperationRecord model.SysOperationRecord, err error) {
	err = global.SHOP_DB.Where("id = ?", id).First(&sysOperationRecord).Error
	return
}
...
func (operationRecordService *OperationRecordService) GetSysOperationRecordInfoList(info systemReq.SysOperationRecordSearch) (list interface{}, total int64, err error) {
	...
	// 创建db
	db := global.SHOP_DB.Model(&model.SysOperationRecord{})
